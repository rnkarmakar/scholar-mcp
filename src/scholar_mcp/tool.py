...
def getScholarData(url = "https://www.google.com/scholar?q=Quantum+Physics&hl=en"):
    ...
    try:
        ...
        for el in soup.select(".gs_r"):
            try:
                scholar_results.append({
                    ...
                    "versions_link": (
                        "https://scholar.google.com" + el.select("a~ a+ .gs_nph")[0]["href"]
                        if el.select("a~ a+ .gs_nph")[0].text else ""
                    )
                })
        ...
        print("Error in getScholarData():", e)
...
@mcp.tool()
def getScholarProfiles(url = "https://scholar.google.com/citations?hl=en&view_op=search_authors&mauthors=Quantum+Physics"):
    """
    ...
    try:
        ...
        for el in soup.select(".gsc_1usr"):
            profile = {
                "name": el.select_one(".gs_ai_name").get_text() if el.select_one(".gs_ai_name") else "",
                "name_link": (
                    "https://scholar.google.com" + el.select_one(".gs_ai_name a")["href"]
                    if el.select_one(".gs_ai_name a") else ""
                ),
                "position": el.select_one(".gs_ai_aff").get_text() if el.select_one(".gs_ai_aff") else "",
                "email": el.select_one(".gs_ai_eml").get_text() if el.select_one(".gs_ai_eml") else "",
                "departments": el.select_one(".gs_ai_int").get_text() if el.select_one(".gs_ai_int") else "",
                "cited_by_count": (
                    el.select_one(".gs_ai_cby").get_text().split()[-1]
                    if el.select_one(".gs_ai_cby") else ""
                )
            }
            # Remove empty fields
        ...
        print("Error in getScholarProfiles():", e)

@mcp.tool()
def getAuthorProfileData(url = "https://scholar.google.com/citations?hl=en&user=cOsxSDEAAAAJ"):
    """
    ...
    try:
        ...
        # Main Author Details
        author_results = {}
        author_results["name"] = soup.select_one("#gsc_prf_in").text if soup.select_one("#gsc_prf_in") else ""
        author_results["position"] = (
            soup.select_one("#gsc_prf_inw + .gsc_prf_il").text
            if soup.select_one("#gsc_prf_inw + .gsc_prf_il") else ""
        )
        author_results["email"] = soup.select_one("#gsc_prf_ivh").text if soup.select_one("#gsc_prf_ivh") else ""
        author_results["departments"] = soup.select_one("#gsc_prf_int").text if soup.select_one("#gsc_prf_int") else ""
        ...
        for el in soup.select("#gsc_a_b .gsc_a_t"):
            article = {
                ...
                "link": "https://scholar.google.com" + el.select_one(".gsc_a_at").get("href", ""),
                "authors": (
                    el.select_one(".gsc_a_at + .gs_gray").text
                    if el.select_one(".gsc_a_at + .gs_gray") else ""
                ),
                "publication": (
                    el.select_one(".gs_gray + .gs_gray").text
                    if el.select_one(".gs_gray + .gs_gray") else ""
                )
            }
        ...
        row1["citations"] = {
            "all": (
                ...
                if soup.select_one("tr:nth-child(1) .gsc_rsb_sc1 + .gsc_rsb_std") else ""
            ),
            "since_2017": (
                soup.select_one("tr:nth-child(1) .gsc_rsb_std + .gsc_rsb_std").text
                if soup.select_one("tr:nth-child(1) .gsc_rsb_std + .gsc_rsb_std") else ""
            )
        }
        ...
        row2["h_index"] = {
            "all": (
                ...
                if soup.select_one("tr:nth-child(2) .gsc_rsb_sc1 + .gsc_rsb_std") else ""
            ),
            "since_2017": (
                soup.select_one("tr:nth-child(2) .gsc_rsb_std + .gsc_rsb_std").text
                if soup.select_one("tr:nth-child(2) .gsc_rsb_std + .gsc_rsb_std") else ""
            )
        }
        ...
        row3["i_index"] = {
            "all": (
                ...
                if soup.select_one("tr~ tr+ tr .gsc_rsb_sc1 + .gsc_rsb_std") else ""
            ),
            "since_2017": (
                soup.select_one("tr~ tr+ tr .gsc_rsb_std + .gsc_rsb_std").text
                if soup.select_one("tr~ tr+ tr .gsc_rsb_std + .gsc_rsb_std") else ""
            )
        }
        ...
        print("Error in getAuthorProfileData():", e)
